...
const DEFAULT_OPTIONS = {
  ...
  blockAudios: true,
  headless: false,
  browserLoadMaxTime: 70000,
  maxParallelWorkers: 2,
  scrollAndClickTarget: '.no-more',
  scrollAndClickButton: '.load-more',
  secondaryScrollAndClickTarget: '.without-comment',
  nonScrollButtonSelector: '.download '
}
const STATE_PROCESSING = 'processing'
const STATE_PROCESSED = 'processed'

const { readTextFile, writeTextFile, writeFile, stdout, mkdir, stat, errors } =
  Deno
let tasks = [],
  maxParallelWorkers,
  sessionFilename
...
async function initialize (options) {
  ...
  if (options.crawlSyncSession || options.crawlLoadSession) {
    try {
      tasks = JSON.parse(
        await readTextFile(options.crawlSyncSession || options.crawlLoadSession)
      )
    } catch (error) {
